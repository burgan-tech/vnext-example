...
Join verdict'i doğrudan instance'ın kaderi olarak gözlemleniyor: case state'inde tek onEntry task'ı
(batch) ve koşulsuz tek bir auto transition var, akışın **hiçbir seviyesinde errorBoundary yok**.
Başarılı join → `case-settled`; başarısız join → **Faulted**. Akışa boundary eklemek başarısız-join
case'lerinin tamamını sessizce "başarılı"ya çevirir — eklenmemeli.
