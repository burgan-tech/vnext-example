...
**Seed layout** — `etc/docker/config/seed/{domain}-collection.json`, one collection per business domain (current set: `account-opening-collection`, `money-transfer-collection`, `future-pay-collection`, `integration-test-collection`, `notification-collection`, `payments-collection`). MockLab recursively scans the seed directory on startup and imports each `*.json` file as a collection.
