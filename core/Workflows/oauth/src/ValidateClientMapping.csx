...
/// </summary>
public class ValidateClientMapping : ScriptBase, IMapping
{
    public Task<ScriptResponse> InputHandler(WorkflowTask task, ScriptContext context)
    {
        try
        {
            LogInformation("Client Validation Input Handler");
            LogInformation("Status: {status}", args: new object?[] { context.Instance?.Data?.status });
            LogInformation("Request ClientId: {ClientId}", args: new object?[] { GetPropertyValue<string>(context.Body, "client_id") });
        ...
        try
        {
            ...
            if (statusCode >= 200 && statusCode < 300)
            {
                ...
                return new ScriptResponse
                {
                    ...
                    Data = new
                    {
                        status = "client-validation-success",
