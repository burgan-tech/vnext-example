...
/// <summary>
/// Notify Document Rejected Mapping - Notifies parent workflow that document is rejected
/// </summary>
public class NotifyDocumentRejectedMapping : IMapping
{
    public Task<ScriptResponse> InputHandler(WorkflowTask task, ScriptContext context)
    {
        try
        {
            ...
            directTriggerTask.SetTransitionName("document-rejected");
            ...
            var transitionBody = new
            {
                ...
                rejectedBy = context.Body?.rejectedBy ?? "user",
                rejectionReason = context.Body?.reason ?? "No reason provided",
                status = "rejected"
            };
        ...
        catch (Exception ex)
        {
            return Task.FromResult(new ScriptResponse
            {
                Key = "notify-rejected-input-error",
        ...
        try
        {
            ...
            if (response?.isSuccess == true)
            {
                return new ScriptResponse
                {
                    ...
                    Tags = new[] { "contract", "document-rejected", "notified" }
                };
            }

            return new ScriptResponse
            {
                Key = "notify-rejected-failed",
                Data = new
                {
