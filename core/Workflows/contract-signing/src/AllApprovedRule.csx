...
// Every document has been approved.
// Counts per-document stamps ("ap_{documentId}") instead of a shared counter, because
// concurrent fan-in callbacks lose increments on a shared read-modify-write key.
public class AllApprovedRule : ScriptBase, IConditionMapping
{
    public Task<bool> Handler(ScriptContext context)
    {
        ...
        {
            foreach (var kv in d)
                if (kv.Key.StartsWith("ap_", StringComparison.Ordinal)
