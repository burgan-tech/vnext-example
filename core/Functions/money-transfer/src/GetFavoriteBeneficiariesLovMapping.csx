...
public class GetFavoriteBeneficiariesLovMapping : ScriptBase, IMapping
{
    public Task<ScriptResponse> InputHandler(WorkflowTask task, ScriptContext context)
    {
        ...
        catch (Exception ex)
        {
            return Task.FromResult(new ScriptResponse
            {
                Key = "favorite-beneficiaries-lov-input-error",
    ...
    public Task<ScriptResponse> OutputHandler(ScriptContext context)
    {
        try
        {
            ...
            if (statusCode >= 200 && statusCode < 300 && items != null)
            {
                return Task.FromResult(new ScriptResponse
                {
                    Key = "favorite-beneficiaries-lov",
                    Data = new { data = items },
            ...
            return Task.FromResult(new ScriptResponse
            {
                Key = "favorite-beneficiaries-lov-failure",
        ...
        catch (Exception ex)
        {
            return Task.FromResult(new ScriptResponse
            {
                Key = "favorite-beneficiaries-lov-exception",
                Data = new { error = ex.Message },
